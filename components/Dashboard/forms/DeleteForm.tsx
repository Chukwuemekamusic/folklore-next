...
export default function DeleteForm({storyId, authorId, storyTitle}: {storyId: string, authorId: string, storyTitle: string}) {
  ...
  const onSubmit = async (data: any) => {
    try {
    const actionResult = await deleteStoryAction2(data.storyId, data.authorId);
    ...
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle className="text-2xl font-bold mb-3 text-center">
          ...
          <span className="text-primary text-xl text-slate-500 block mt-2">"{storyTitle.toUpperCase()}"</span>
        </CardTitle>
